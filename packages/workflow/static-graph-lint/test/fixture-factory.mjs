import { computeDescriptorFingerprints, computeRootIntegrity } from "../src/index.mjs";
import { FORBIDDEN_PARTNER_CLASS_TYPES } from "../src/constants.mjs";
export { documents } from "../src/certified-h3-documents.mjs";
...
function descriptorBase(classType) {
  ...
  throw new Error("unknown fixture class");
}
...
export function resign(document) {
  document.integrity.content_sha256 = computeRootIntegrity(document);
  return document;
}

export function positiveApiGraph(kind = "api") {
  return {
    nodes: {
      source: { subgraph_ref: "model-source", inputs: {}, _meta: { title: "typed local interface" } },
      sigma: {
        class_type: "MiniMaxH3SigmaShift",
        inputs: { model: ["source", 0], shift_video: 12, shift_audio: 3 },
        _meta: { title: "local sigma shift" },
      },
    },
    definitions: {
      subgraphs: [{ id: "model-source", kind, inputs: [], outputs: [{ name: "model", type: "MODEL" }], graph: {} }],
    },
  };
}

export function positiveVisualGraph() {
  return {
    version: 0.4,
    nodes: [{
      id: 1,
